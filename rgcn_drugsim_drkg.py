...
class Net(torch.nn.Module):
    def __init__(self, dropout):
        ...
        self.drug_emb = Parameter(torch.randn(2643, 400))
        self.gene_emb = Parameter(torch.randn(4264, 2013))
        self.conv1 = RGCNConv(2013, 2000, 5)
        self.conv2 = RGCNConv(2000, 1000, 5)
        self.lin1 = Linear(1000, 500)
        self.lin2 = Linear(500, 2)
        self.dropout = dropout

    def forward(self, x, x1, edge_index, edge_type):
        x1 = torch.cat((x1[:1228], self.drug_emb, x1[1228:]), dim=0)
...
def cv_train():
    model.train()
    optimizer.zero_grad()
    out = model(data.x, data.x1, data.edge_index, data.edge_type)
...
def cv_test():
    model.eval()
    out = model(data.x, data.x1, data.edge_index, data.edge_type).exp()[
...
def test():
    model.eval()
    out = model(data.x, data.x1, data.edge_index,
                data.edge_type).exp()[test_index]
    pred = out.argmax(dim=-1)
...
data = torch.load('graph_drugsim_drkg.pt')
